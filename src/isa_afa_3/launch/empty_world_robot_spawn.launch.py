...
def generate_launch_description():
    ...
    def generate_ros_gz_bridge_arg(
        topic_name: str, ros_msg_type: str, gz_msg_type: str, mode="bi"
    ):
        mode_mapping = {"bi": "@", "uni_from_ros": "]", "uni_from_gz": "["}

        try:
            return f"{topic_name}@{ros_msg_type}{mode_mapping[mode]}{gz_msg_type}"
        except KeyError:
            print(
                "ERROR: Valid values for mode are 'bi', 'uni_from_ros' or 'uni_from_gz'"
            )
    ...
    gz_bridge = Node(
        ...
        name="gz_bridge",
        arguments=[
            generate_ros_gz_bridge_arg(
                "/clock", "rosgraph_msgs/msg/Clock", "gz.msgs.Clock", "uni_from_gz"
            ),
            generate_ros_gz_bridge_arg(
